...
export function MyGroupsResultsComponent(props) {
  ...
  const currentGroups = useSelector((state) => {
    return state.groups.mine.map((id) => state.groups.data[id]).filter((group) => group.date > new Date().toISOString());
  });
  ...
  return currentGroups.length > 0 || pastGroups.length > 0 ? (
    <div>
      ...
      {currentGroups.length === 0 ? (
          <center><div>You do not have any active groups</div></center>
        )
          : (
